# FR-03: 新形状渲染

> 优先级: P1 | 状态: ✅ 完成

## 📋 概述

为 FR-01 解析的 30+ 新形状类型实现 SVG 渲染.

参考: [Mermaid Flowchart - Node Shapes](https://mermaid.js.org/syntax/flowchart.html#node-shapes)

---

## 🎯 需求

### 3.1 渲染优先级

按使用频率分批实现:

**第一批 (常用):**
- `doc` / `document` - 文档形状 (底部波浪)
- `card` / `notched-rectangle` - 卡片/缺角矩形
- `delay` / `half-rounded-rect` - 延迟/半圆角
- `hourglass` / `collate` - 沙漏/整理

**第二批 (中等频率):**
- `bolt` / `lightning` - 闪电
- `triangle` / `tri` - 三角形
- `window-pane` / `win-pane` - 窗格
- `divided-rect` / `div-rect` - 分隔矩形
- `lined-rect` / `lin-rect` - 带线矩形

**第三批 (其他):**
- `fork` - 分叉/合并
- `crossed-circle` - 叉圈
- `sm-circ` / `small-circle` - 小圆
- 其他 shapes.ts 中定义的形状

### 3.2 SVG 路径实现

每个形状需要:
1. SVG path 数据生成函数
2. 支持动态宽高
3. 与现有渲染管线集成

---

## 🔧 实现

### ShapeRenderer 扩展

**文件:** `src/canvas/shapes/ShapeRenderer.ts`

```typescript
// 新形状渲染映射
const NEW_SHAPE_RENDERERS: Record<string, ShapeRenderFn> = {
  doc: renderDocument,
  card: renderCard,
  delay: renderDelay,
  hourglass: renderHourglass,
  bolt: renderBolt,
  triangle: renderTriangle,
  // ...
};

// 文档形状 (底部波浪)
function renderDocument(x: number, y: number, w: number, h: number): string {
  const waveHeight = h * 0.1;
  return `
    M ${x} ${y}
    L ${x + w} ${y}
    L ${x + w} ${y + h - waveHeight}
    Q ${x + w * 0.75} ${y + h} ${x + w / 2} ${y + h - waveHeight}
    Q ${x + w * 0.25} ${y + h - waveHeight * 2} ${x} ${y + h - waveHeight}
    Z
  `;
}

// 卡片/缺角矩形
function renderCard(x: number, y: number, w: number, h: number): string {
  const notch = Math.min(w, h) * 0.15;
  return `
    M ${x + notch} ${y}
    L ${x + w} ${y}
    L ${x + w} ${y + h}
    L ${x} ${y + h}
    L ${x} ${y + notch}
    Z
  `;
}
```

### 参考 Mermaid 源码

**文件:** `docs/source_ref/mermaid/packages/mermaid/src/rendering-util/rendering-elements/shapes/`

Mermaid 的形状实现位于上述目录,每个形状一个文件.

---

## ✅ 验收标准

### 渲染测试

```typescript
describe('新形状渲染', () => {
  it('渲染 document 形状', () => {
    const svg = renderShape('doc', 100, 100, 120, 80);
    expect(svg).toContain('Q'); // 包含二次曲线 (波浪)
  });

  it('渲染 card 形状', () => {
    const svg = renderShape('card', 100, 100, 120, 80);
    // 验证缺角路径
  });
});
```

### E2E 测试

```typescript
// e2e/shapes.spec.ts
test('新形状正确渲染', async ({ page }) => {
  // 输入 Mermaid 代码
  await page.fill('[data-testid="code-input"]', `
    flowchart TD
      A@{ shape: doc, label: "文档" }
  `);

  // 验证 SVG 存在
  await expect(page.locator('svg path')).toBeVisible();
});
```

---

## 📊 工作量估算

| 形状数量 | 预计时间 |
|----------|----------|
| 第一批 (4个) | 2-3 天 |
| 第二批 (5个) | 2-3 天 |
| 第三批 (20+) | 5-7 天 |

---

## 🔗 依赖

- 前置: FR-01 (新形状语法解析)
- 被依赖: FR-10 (形状选择器优化)
