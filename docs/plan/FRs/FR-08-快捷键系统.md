...
**新文件:** `src/lib/HotkeyManager.ts`

```typescript
interface HotkeyBinding {
  keys: string;                    // 如 "ctrl+z", "cmd+shift+z"
  action: () => void;
  context?: string;                // 上下文,如 'canvas', 'editor', 'dialog'
  description?: string;
...
**组件:** `src/components/HotkeyHelp.svelte`
...
- 前置: FR-05 (撤销重做), FR-06 (节点交互), FR-07 (边交互)
- 被依赖: 无
