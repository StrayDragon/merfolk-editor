# FR-02: 高级边语法

> 优先级: P0 | 状态: ⬜ 待开始

## 📋 概述

增强边(Edge)解析能力，支持所有 Mermaid 边语法变体。

参考: [Mermaid Flowchart - Links](https://mermaid.js.org/syntax/flowchart.html#links-between-nodes)

---

## 🎯 需求

### 2.1 空格分隔的文本语法 (当前失败 ❌)

**语法:**
```mermaid
A -- text --> B     %% 空格分隔文本 + 箭头
A -- text --- B     %% 空格分隔文本 + 无箭头
A -. text .-> B     %% 虚线 + 空格文本
A == text ==> B     %% 粗线 + 空格文本
```

**当前问题:** 解析器未能正确提取空格分隔的边文本。

### 2.2 不可见边 (待实现)

**语法:**
```mermaid
A ~~~ B
```

用于布局调整，不渲染可见连线。

### 2.3 多节点链接 & (待实现)

**语法:**
```mermaid
A & B --> C         %% A-->C, B-->C (2 条边)
A --> B & C         %% A-->B, A-->C (2 条边)
A & B --> C & D     %% 笛卡尔积 (4 条边)
```

### 2.4 边长度控制 (待实现)

**语法:**
```mermaid
A --> B             %% 默认长度 1
A ---> B            %% 长度 2
A ----> B           %% 长度 3
A -.-> B            %% 虚线默认长度 1
A -..-> B           %% 虚线长度 2
A ==> B             %% 粗线默认长度 1
A ===> B            %% 粗线长度 2
```

### 2.5 边 ID 和动画 (待实现)

**边 ID 语法:**
```mermaid
A e1@--> B          %% 边 ID 为 e1
```

**边属性配置:**
```mermaid
A e1@--> B
e1@{ animate: true }
e1@{ animation: fast }  %% slow, normal, fast
```

### 2.6 linkStyle (待实现)

**语法:**
```mermaid
linkStyle 0 stroke:#ff3,stroke-width:4px
linkStyle 1,2 stroke:#00f
linkStyle default stroke:#333
```

---

## 🔧 实现

### Parser 修改

**文件:** `src/core/parser/MermaidParser.ts`

#### 2.1 修复空格文本解析

当前正则无法匹配 `A -- text --> B` 格式。

**修改 `splitByEdges`:**

```typescript
// 需要处理的边类型正则
const EDGE_PATTERNS = {
  // 带文本的空格语法 (优先匹配)
  textNormal: /--\s+(.+?)\s+-->/,      // A -- text --> B
  textThick: /==\s+(.+?)\s+==>/,       // A == text ==> B
  textDotted: /-\.\s*(.+?)\s*\.->/,    // A -. text .-> B
  // 无文本的边
  normal: /-->/,
  thick: /==>/,
  dotted: /-\.->|--\.>/,
  // ...
};
```

#### 2.2 添加不可见边

```typescript
// 添加 invisible 边类型
const INVISIBLE_EDGE = /~~~/;
```

#### 2.3 多节点链接展开

```typescript
private expandMultiNodeLinks(line: string): string[] {
  // 检测 & 操作符
  // 例: "A & B --> C & D"
  // 展开为: ["A --> C", "A --> D", "B --> C", "B --> D"]
}
```

#### 2.4 边长度计算

```typescript
private parseEdgeLength(operator: string): number {
  // 统计 - 或 = 的数量
  // --> = 1, ---> = 2, ----> = 3
  const dashes = operator.match(/-+/)?.[0].length ?? 2;
  return Math.max(1, dashes - 1);
}
```

### Model 修改

**文件:** `src/core/model/Edge.ts`

添加新属性:
```typescript
interface EdgeData {
  // 现有
  id: string;
  source: string;
  target: string;
  text?: string;
  stroke: StrokeType;
  arrowStart: ArrowType;
  arrowEnd: ArrowType;
  // 新增
  length?: number;           // 边长度
  isUserDefinedId?: boolean; // 是否用户定义 ID
  animate?: boolean;         // 是否动画
  animation?: 'slow' | 'normal' | 'fast';
  style?: Record<string, string>; // linkStyle
}
```

---

## ✅ 验收标准

### 测试用例

```typescript
// 2.1 空格文本语法
it('解析 A -- text --> B', () => {
  const model = parser.parse('flowchart LR\n  A -- text --> B');
  expect(model.edges[0].text).toBe('text');
  expect(model.edges[0].arrowEnd).toBe('arrow');
});

it('解析 A -. text .-> B', () => {
  const model = parser.parse('flowchart LR\n  A -. text .-> B');
  expect(model.edges[0].text).toBe('text');
  expect(model.edges[0].stroke).toBe('dotted');
});

// 2.2 不可见边
it('解析不可见边 ~~~', () => {
  const model = parser.parse('flowchart LR\n  A ~~~ B');
  expect(model.edges[0].stroke).toBe('invisible');
});

// 2.3 多节点链接
it('A & B --> C 展开为 2 条边', () => {
  const model = parser.parse('flowchart LR\n  A & B --> C');
  expect(model.edgeCount).toBe(2);
  expect(model.edges.find(e => e.source === 'A' && e.target === 'C')).toBeDefined();
  expect(model.edges.find(e => e.source === 'B' && e.target === 'C')).toBeDefined();
});

// 2.4 边长度
it('边长度正确计算', () => {
  const model = parser.parse('flowchart TD\n  A ---> B\n  C ----> D');
  expect(model.edges[0].length).toBe(2);
  expect(model.edges[1].length).toBe(3);
});

// 2.5 边 ID
it('边 ID 语法', () => {
  const model = parser.parse('flowchart LR\n  A e1@--> B');
  expect(model.edges[0].id).toBe('e1');
  expect(model.edges[0].isUserDefinedId).toBe(true);
});
```

---

## 📊 当前失败测试

来自 `MermaidParser.compat.test.ts`:

| 测试 | 问题 |
|------|------|
| ❌ 3.4 带文本的边 -- text --> | `text` 未被解析 |
| ❌ 3.6 带文本虚线边 -. text .-> | `text` 未被解析 |
| ❌ 3.8 带文本粗线边 == text ==> | `text` 未被解析 |
| ❌ 4.2 带文本的节点链 | 链式边的 `text` 未被解析 |

---

## 🔗 依赖

- 无前置依赖
- 被依赖: FR-07 (边交互优化)
