# FR-05: 撤销/重做系统

> 优先级: P0 | 状态: ✅ 完成

## 📋 概述

实现可靠的撤销(Undo)和重做(Redo)系统,支持所有编辑操作.

---

## 🎯 需求

### 5.1 支持的操作

- 添加/删除节点
- 添加/删除边
- 修改节点属性 (文本、形状、样式)
- 修改边属性 (类型、文本)
- 移动节点位置
- 添加/删除子图
- 批量操作 (多选后操作)

### 5.2 用户交互

- `Ctrl+Z` / `Cmd+Z` - 撤销
- `Ctrl+Y` / `Cmd+Shift+Z` - 重做
- 工具栏按钮 (可选)

### 5.3 历史栈限制

- 默认保存 50 步
- 可配置

---

## 🔧 实现

### Command 模式

**新文件:** `src/core/command/`

```
src/core/command/
├── Command.ts          # 命令基类
├── CommandHistory.ts   # 历史管理器
├── commands/
│   ├── AddNodeCommand.ts
│   ├── DeleteNodeCommand.ts
│   ├── AddEdgeCommand.ts
│   ├── DeleteEdgeCommand.ts
│   ├── UpdateNodeCommand.ts
│   ├── UpdateEdgeCommand.ts
│   ├── MoveNodeCommand.ts
│   └── CompositeCommand.ts  # 批量操作
└── index.ts
```

**Command 接口:**
```typescript
interface Command {
  execute(): void;
  undo(): void;
  description?: string;
}

class CommandHistory {
  private undoStack: Command[] = [];
  private redoStack: Command[] = [];
  private maxSize: number = 50;

  execute(cmd: Command): void {
    cmd.execute();
    this.undoStack.push(cmd);
    this.redoStack = []; // 清空重做栈
  }

  undo(): boolean {
    const cmd = this.undoStack.pop();
    if (cmd) {
      cmd.undo();
      this.redoStack.push(cmd);
      return true;
    }
    return false;
  }

  redo(): boolean {
    const cmd = this.redoStack.pop();
    if (cmd) {
      cmd.execute();
      this.undoStack.push(cmd);
      return true;
    }
    return false;
  }

  get canUndo(): boolean { return this.undoStack.length > 0; }
  get canRedo(): boolean { return this.redoStack.length > 0; }
}
```

**AddNodeCommand 示例:**
```typescript
class AddNodeCommand implements Command {
  constructor(
    private model: FlowchartModel,
    private data: NodeData
  ) {}

  execute(): void {
    this.model.addNode(this.data);
  }

  undo(): void {
    this.model.removeNode(this.data.id);
  }
}
```

### FlowchartModel 集成

不直接调用 `model.addNode()`,而是通过 CommandHistory:

```typescript
// 编辑器中
function addNode(data: NodeData) {
  history.execute(new AddNodeCommand(model, data));
}
```

### 快捷键绑定

**文件:** `src/components/Editor.svelte`

```svelte
<script>
  function handleKeydown(e: KeyboardEvent) {
    if ((e.ctrlKey || e.metaKey) && e.key === 'z') {
      e.preventDefault();
      if (e.shiftKey) {
        history.redo();
      } else {
        history.undo();
      }
    }
    if ((e.ctrlKey || e.metaKey) && e.key === 'y') {
      e.preventDefault();
      history.redo();
    }
  }
</script>

<svelte:window on:keydown={handleKeydown} />
```

---

## ✅ 验收标准

### 测试用例

```typescript
describe('CommandHistory', () => {
  it('执行后可撤销', () => {
    const history = new CommandHistory();
    const model = new FlowchartModel();

    history.execute(new AddNodeCommand(model, { id: 'A', text: 'A', shape: 'rect' }));
    expect(model.hasNode('A')).toBe(true);

    history.undo();
    expect(model.hasNode('A')).toBe(false);
  });

  it('撤销后可重做', () => {
    const history = new CommandHistory();
    const model = new FlowchartModel();

    history.execute(new AddNodeCommand(model, { id: 'A', text: 'A', shape: 'rect' }));
    history.undo();
    history.redo();

    expect(model.hasNode('A')).toBe(true);
  });

  it('新操作清空重做栈', () => {
    const history = new CommandHistory();
    const model = new FlowchartModel();

    history.execute(new AddNodeCommand(model, { id: 'A', text: 'A', shape: 'rect' }));
    history.undo();
    history.execute(new AddNodeCommand(model, { id: 'B', text: 'B', shape: 'rect' }));

    expect(history.canRedo).toBe(false);
  });
});
```

### E2E 测试

```typescript
test('Ctrl+Z 撤销节点添加', async ({ page }) => {
  // 添加节点
  await page.click('[data-testid="add-node-btn"]');
  await expect(page.locator('.node')).toHaveCount(1);

  // 撤销
  await page.keyboard.press('Control+z');
  await expect(page.locator('.node')).toHaveCount(0);
});
```

---

## 🔗 依赖

- 无前置依赖
- 被依赖: FR-06, FR-07 (需要通过命令执行操作)
